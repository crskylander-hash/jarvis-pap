...
| 20 | Seletor de idioma da conversa (pt-PT, en-GB, es-ES, fr-FR) + vozes filtradas por idioma + exportação da conversa (.txt) | O idioma muda o reconhecimento de voz, a voz sintetizada E a língua da resposta (campo `idioma` no POST /chat, opcional e retrocompatível — o RPi continua a funcionar sem alterações). A interface da app mantém-se sempre em PT-PT. |
